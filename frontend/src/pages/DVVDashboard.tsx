...
export default function DVVDashboard() {
    const { user, isLoading: authLoading } = useAuth();
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('DVV_VERIFIER' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('DVV_VERIFIER');
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`DVV Verification: ${user.fullName || 'Verifier'}`}
                    description="Data Validation & Verification portal for institutional compliance evidence."
