...
export default function ExternalReviewerDashboard() {
    ...
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('EXTERNAL_REVIEWER' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('EXTERNAL_REVIEWER');
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`External Peer Review: ${user.fullName || 'Reviewer'}`}
                    description="Independent peer review portal for institutional assessment and qualitative metrics."
