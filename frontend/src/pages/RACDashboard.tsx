...
export default function RACDashboard() {
    const { user, isLoading: authLoading } = useAuth();
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('RAC_MEMBER' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('RAC_MEMBER');
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`RAC Panel: ${user.fullName || 'Member'}`}
                    description="Research Advisory Committee dashboard for scholar progress tracking and milestone reviews."
            ...
            <DashboardLayout
                ...
                onWidgetAction={(widgetId, actionId) => {
                    if (widgetId === 'scholars_progress_overview') {
                        // Navigate to a dedicated review page (using PhDScrutiny as proxy or similar)
                        window.location.href = `/app/iers/rac/review?instanceId=${actionId}`;
                    }
                }}
            />
        </div>
