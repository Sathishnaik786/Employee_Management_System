...
export default function IQACDashboard() {
    ...
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('IQAC_MEMBER' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('IQAC_MEMBER');
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`IQAC Compliance: ${user.fullName || 'Member'}`}
                    description="Institutional Quality Assurance Cell dashboard for NAAC SSR and IIQA management."
