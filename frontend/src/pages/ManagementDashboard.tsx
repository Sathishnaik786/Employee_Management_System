...
export default function ManagementDashboard() {
    ...
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('MANAGEMENT' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('MANAGEMENT');
    ...
    // Feature Flag Check: ManagementDashboardV2
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`Strategic Research Center: ${user.fullName || 'Member'}`}
                    description="Institution-wide analytics and strategic KPI monitoring for governance."
