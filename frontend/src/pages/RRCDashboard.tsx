...
export default function RRCDashboard() {
    const { user, isLoading: authLoading } = useAuth();
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('RRC_MEMBER' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('RRC_MEMBER');
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`RRC Panel: ${user.fullName || 'Member'}`}
                    description="Research Review Committee portal for doctoral synopsis and thesis oversight."
