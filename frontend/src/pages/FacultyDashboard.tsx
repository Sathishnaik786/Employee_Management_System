...
export default function FacultyDashboard() {
  ...
  const dashboardConfig = useMemo(() => {
    if (!user) return null;
    return getDashboardConfig('FACULTY');
  }, [user]);
  ...
  return (
    <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
      <motion.div variants={slideUpVariants} initial="initial" animate="animate">
        <PageHeader
          title={`Welcome, ${user.fullName || 'Faculty Member'}`}
          description="Faculty academic overview and supervision dashboard with permission-based access."
