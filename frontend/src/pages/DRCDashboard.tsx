...
export default function DRCDashboard() {
    ...
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('DRC_MEMBER' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('DRC_MEMBER');
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`DRC Panel: ${user.fullName || 'Member'}`}
                    description="Departmental Research Committee dashboard for PhD scrutiny, interviews, and lifecycle management."
            ...
            <DashboardLayout
                ...
                onWidgetAction={(widgetId, actionId) => {
                    if (widgetId === 'scrutiny_list') {
                        navigate(`/app/iers/drc/scrutiny?instanceId=${actionId}`);
                    }
                }}
            />
        </div>
