...
export default function AdjudicatorDashboard() {
    ...
    const dashboardConfig = useMemo(() => {
        if (!user) return null;
        return getDashboardConfig('ADJUDICATOR' as any);
    }, [user]);

    const { data, loading: dataLoading } = useDashboardData('ADJUDICATOR');
    ...
    return (
        <div className="p-6 lg:p-10 space-y-10 max-w-[1600px] mx-auto">
            <motion.div variants={slideUpVariants} initial="initial" animate="animate">
                <PageHeader
                    title={`Adjudicator Workspace: ${user.fullName || 'Member'}`}
                    description="Isolated thesis evaluation workspace with secure report upload endpoints."
